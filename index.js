...
// ─── PREDICTION ENGINE (walk-forward validated) ────────────────────────────────
...
function sigEntropy(sub) {
  ...
  const sr = sw / 14;
  if (sr > 0.70) return w[w.length-1] === 'TAI' ? 'XIU' : 'TAI';
  if (sr < 0.35) return w[w.length-1];
  return null;
}
...
function sigMeanReversion(sub) {
  ...
  if (taiCount >= 22) return 'XIU';
  if (taiCount <= 8)  return 'TAI';
  return null;
}
...
function sigAltBreak(sub) {
  ...
  if (altLen >= 6) return sub[sub.length-1];
  return null;
}
...
// Khác với backtest thường (test trên chính data đã thấy), walk-forward chia:
//   - train window: để signal "nhìn thấy" và tính pattern
//   - test window: signal PHẢI dự đoán mà KHÔNG được nhìn trước — giống thực chiến
// Điều này cho ra % trung thực hơn nhiều so với in-sample backtest.
function walkForwardValidate(seq, fn, foldSize = 20, minFolds = 3) {
  const n = seq.length;
  if (n < foldSize * (minFolds + 1)) return null; // cần đủ ít nhất minFolds+1 folds

  let hits = 0, total = 0;
  // Trượt cửa sổ: mỗi điểm i chỉ được dùng history TRƯỚC nó để dự đoán
  // rồi so sánh với kết quả THẬT tại i (chưa từng "nhìn thấy" lúc tính hàm fn)
  const start = Math.max(foldSize, n - foldSize * 8); // giới hạn để không quá chậm
  for (let i = start; i < n; i++) {
    const trainSeq = seq.slice(0, i); // chỉ dùng quá khứ
    const pred = fn(trainSeq);
    if (pred === null) continue;
    total++;
    if (pred === seq[i]) hits++;
  }
  if (total < foldSize) return null;

  // Chia đôi để kiểm tra tính ổn định (consistency) qua thời gian
  const half = Math.floor(total / 2);
  // (đơn giản hoá: coi acc tổng thể, ưu tiên minh bạch hơn phức tạp hoá)
  const acc = hits / total;

  // Wilson score interval - phạt các mẫu nhỏ, tránh ảo tưởng % cao do ít mẫu
  const z = 1.96; // 95% confidence
  const nTotal = total;
  const phat = acc;
  const denominator = 1 + z*z/nTotal;
  const centre = phat + z*z/(2*nTotal);
  const spread = z * Math.sqrt((phat*(1-phat) + z*z/(4*nTotal))/nTotal);
  const lowerBound = (centre - spread) / denominator; // cận dưới thực tế, khiêm tốn hơn acc thô

  return { acc, total, lowerBound };
}
...
// Dự đoán chính: chỉ dùng signal có lowerBound (cận dưới Wilson) > 50%
// tức là ngay cả trong trường hợp xấu nhất về mặt thống kê, signal vẫn có edge thật
function predictNext(history) {
  const seq = [...history];
  if (seq.length < 30) {
    return { pred: 'TAI', conf: 50, signals: [], n_active: 0, regime: 'INIT', reason: 'Cần thêm dữ liệu để walk-forward validate' };
  }
  ...
  for (const [tag, fn, capW] of SIGNALS) {
    const wf = walkForwardValidate(seq, fn);
    if (!wf) continue;
    // Điều kiện chặt: cận dưới Wilson (không phải acc thô) phải > 51%
    if (wf.lowerBound <= 0.51) continue;
    const cur = fn(seq);
    if (cur === null) continue;
    const sampleConf = Math.min(1.0, wf.total / 30);
    const regMult = getRegimeMult(regime, tag);
    const edge = wf.lowerBound - 0.5;
    const weight = edge * 2 * capW * sampleConf * regMult;
    if (weight <= 0.02) continue;
    votes.push([cur === 'TAI' ? 1.0 : 0.0, weight]);
    activeSignals.push(`${tag}:${(wf.acc*100).toFixed(0)}%(min ${(wf.lowerBound*100).toFixed(0)}%)/${wf.total}`);
  }

  if (!votes.length) {
    // Không signal nào đủ tin cậy thật sự → không có edge, trả về trung lập
    return { pred: seq[seq.length-1], conf: 50, signals: [], n_active: 0, regime, reason: 'Không có tín hiệu đủ tin cậy (walk-forward) — theo cầu mặc định' };
  }
  ...
  // Confidence khiêm tốn hơn: cap ở mức thực tế cho game 50/50
  const conf = Math.round(50 + Math.min(edge / 0.30, 1.0) * 12); // max ~62%, không phải 95%
  return { pred, conf, p_tai: +pTai.toFixed(3), n_active: votes.length, signals: activeSignals, regime, reason: `${votes.length} tín hiệu walk-forward đồng thuận` };
}
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div style="font-size:13px;color:var(--t2);line-height:1.6">
        <div style="margin-bottom:8px">🧠 <span style="color:var(--t1);font-weight:600">Walk-Forward Validated Engine</span></div>
        <div>• 22 thuật toán: Markov, Pattern, Bias, Zigzag, Chu kỳ...</div>
        <div>• Kiểm định walk-forward: mỗi tín hiệu chỉ được tính khi thực sự dự đoán đúng trên dữ liệu <b>chưa từng thấy</b>, không phải backtest trên chính nó</div>
        <div>• Chỉ dùng tín hiệu có cận dưới thống kê (Wilson score) &gt;51% — loại bỏ ảo tưởng do mẫu nhỏ</div>
        <div>• Độ tin cậy hiển thị khiêm tốn (tối đa ~62%) để phản ánh đúng bản chất game 50/50</div>
