...
class Lc79Session {
  ...
  _handleEvent(event, data) {
    ...
    if (event === 'your-info') {
      ...
      addLog('info', `📦 won-session: ${JSON.stringify(data).slice(0,120)}`);
      const prize = data.prize ?? data.winAmount ?? data.amount ?? 0;
      const balanceAfter = data.balance ?? data.newBalance ?? data.currentBalance ?? 0;
      if (balanceAfter) this.balance = balanceAfter;
      const won = prize > 0;
      if (this.sessionPlaced || this.lastBetType) {
        if (won) { this.statWin++; this.statProfit += prize; }
        else { this.statLose++; this.statProfit += Math.abs(prize) * -1; }
      }
      const icon = won ? '✅ THẮNG' : '❌ THUA';
      addLog(won ? 'win' : 'lose', `${icon} | ${won ? '+' : '-'}${Math.abs(prize).toLocaleString()}đ | P/L: ${this.statProfit >= 0 ? '+' : ''}${this.statProfit.toLocaleString()}đ`);
      ...
      if (data.type && data.amount != null) {
        ...
        // bet confirm — đã log khi gửi lệnh
        broadcastState();
      }
    }
