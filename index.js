...
// ─── PREDICTION ENGINE (port chính xác từ autolc.py gốc) ───────────────────────

function _sigWeightedMarkov(order, decay = 0.88) {
  return (sub) => {
...
function _sigStreakFollow(sub) {
  if (sub.length < 2) return null;
  return sub[sub.length-1];
}

function _sigStreakBreak(minLen) {
  return (sub) => {
...
function _sigDoubleBreak(minLen) {
  return (sub) => {
    if (sub.length < minLen + 4) return null;
    const last = sub[sub.length-1];
    let curS = 1;
    for (let i = sub.length-2; i >= 0; i--) {
      if (sub[i] === last) curS++; else break;
    }
    if (curS < minLen) return null;
    const idx = sub.length - 1 - curS;
    if (idx < 0) return null;
    const prevVal = sub[idx];
    if (prevVal === last) return null;
    let prevS = 1;
    for (let i = idx-1; i >= 0; i--) {
      if (sub[i] === prevVal) prevS++; else break;
    }
    if (curS <= prevS) return null;
    return last === 'TAI' ? 'XIU' : 'TAI';
  };
}

function _sigPattern(order) {
  return (sub) => {
...
function _sigLocalConsistency(window = 20) {
  return (sub) => {
    if (sub.length < window) return null;
    const w = sub.slice(-window);
    const nTai = w.filter(v => v === 'TAI').length;
    const ratio = nTai / window;
    if (ratio > 0.68) return 'XIU';
    if (ratio < 0.32) return 'TAI';
    return null;
  };
}

function _sigAlternation(runLen) {
  return (sub) => {
    if (sub.length < runLen * 4) return null;
    const runs = [];
    let cv = sub[0], cl = 1;
    for (let i = 1; i < sub.length; i++) {
      if (sub[i] === cv) cl++;
      else { runs.push([cv, cl]); cv = sub[i]; cl = 1; }
    }
    runs.push([cv, cl]);
    if (runs.length < 4) return null;
    const last4 = runs.slice(-4);
    for (let i = 0; i < last4.length - 1; i++) {
      if (last4[i][1] !== runLen) return null;
    }
    const [curVal, curLen] = last4[last4.length-1];
    return curLen < runLen ? curVal : (curVal === 'TAI' ? 'XIU' : 'TAI');
  };
}

function _sigRecency(halflife) {
  return (sub) => {
    if (sub.length < halflife * 2) return null;
    const decay = Math.pow(0.5, 1.0/halflife);
    let wt = 0, wx = 0;
    const n = sub.length;
    for (let i = 0; i < n; i++) {
      const wd = Math.pow(decay, n - i - 1);
      if (sub[i] === 'TAI') wt += wd; else wx += wd;
    }
    const tot = wt + wx;
    if (tot <= 0) return null;
    const rt = wt / tot;
    if (rt > 0.60) return 'XIU';
    if (rt < 0.40) return 'TAI';
    return null;
  };
}

function _sigSwitchRateShort() {
  return (sub) => {
    if (sub.length < 15) return null;
    function sr(w) {
      let c = 0;
      for (let i = 1; i < w.length; i++) if (w[i] !== w[i-1]) c++;
      return c / Math.max(1, w.length-1);
    }
    const sr3 = sr(sub.slice(-3)), sr5 = sr(sub.slice(-5)), sr10 = sr(sub.slice(-10));
    const last = sub[sub.length-1];
    if (sr3 > sr5 && sr5 > sr10 && sr3 > 0.50 && (sr3 - sr10) > 0.20) {
      return last === 'TAI' ? 'XIU' : 'TAI';
    }
    if (sr3 < 0.30 && sr3 < sr10 - 0.25) return last;
    return null;
  };
}

function _sigZigzagPersist() {
  return (sub) => {
    if (sub.length < 8) return null;
    let sr6c = 0;
    for (let i = 1; i <= 6; i++) if (sub[sub.length-i] !== sub[sub.length-i-1]) sr6c++;
    const sr6 = sr6c / 6;
...
function _sigStreakExit() {
  return (sub) => {
    if (sub.length < 12) return null;
    const runs = [];
    let cv = sub[0], cl = 1;
    for (let i = 1; i < sub.length; i++) {
      if (sub[i] === cv) cl++;
    ...
    const [prevVal, prevLen] = runs[runs.length-2];
    const [curVal, curLen] = runs[runs.length-1];
    if (prevLen >= 4 && curLen <= 2) return curVal;
    if (prevLen >= 3 && curLen >= 3) return curVal;
    return null;
  };
}

function _sigMomentumDecay(p80Factor = 0.80) {
  return (sub) => {
    ...
    const rs = [...runs].sort((a,b)=>a-b);
    const p80 = rs[Math.min(rs.length-1, Math.floor(rs.length * p80Factor))];
...
function _detectRegime(seq) {
  if (seq.length < 10) return 'CHOPPY';
  let sr10c = 0;
  const lim = Math.min(11, seq.length);
  for (let i = 1; i < lim; i++) if (seq[seq.length-i] !== seq[seq.length-i-1]) sr10c++;
  const sr10 = sr10c / Math.min(10, seq.length-1);
  let streak = 1;
  for (let i = seq.length-2; i >= 0; i--) {
    if (seq[i] === seq[seq.length-1]) streak++; else break;
  }
  const w20 = seq.length >= 20 ? seq.slice(-20) : seq;
...
function _backtestSplit(seq, fn, window = 120, cap = 200) {
  const n = seq.length;
  ...
  const total = t1 + t2;
  if (total < 10) return [null, total, 0.0];
  const acc = (h1 + h2) / total;
  const acc1 = t1 >= 5 ? h1/t1 : null;
  const acc2 = t2 >= 5 ? h2/t2 : null;
  const consistency = (acc1 !== null && acc2 !== null && acc1 > 0.50 && acc2 > 0.50) ? 1.0 : 0.6;
  return [acc, total, consistency];
}

const _SIGNALS = [
  ['WM1',  _sigWeightedMarkov(1),        2.0],
  ['WM2',  _sigWeightedMarkov(2),        2.5],
  ['WM3',  _sigWeightedMarkov(3),        3.0],
  ['SF',   _sigStreakFollow,             1.5],
  ['SB3',  _sigStreakBreak(3),           2.0],
  ['SB5',  _sigStreakBreak(5),           2.5],
  ['SB7',  _sigStreakBreak(7),           3.0],
  ['DB4',  _sigDoubleBreak(4),           2.8],
  ['DB6',  _sigDoubleBreak(6),           3.2],
  ['P3',   _sigPattern(3),               2.0],
  ['P4',   _sigPattern(4),               2.5],
  ['P5',   _sigPattern(5),               3.0],
  ['B10',  _sigBias(10),                 1.2],
  ['B20',  _sigBias(20),                 1.2],
  ['LC20', _sigLocalConsistency(20),     1.3],
  ['ALT1', _sigAlternation(1),           1.8],
  ['ALT2', _sigAlternation(2),           1.8],
  ['RW8',  _sigRecency(8),               1.4],
  ['RW16', _sigRecency(16),              1.3],
  ['ZPG',  _sigZigzagPersist(),          2.0],
  ['SEG',  _sigStreakExit(),             2.5],
  ['MDK',  _sigMomentumDecay(),          2.2],
  ['SRD',  _sigSwitchRateShort(),        2.0],
];

const _REGIME_MULT = {
  'STREAK:SB': 1.4, 'STREAK:MDK': 1.6, 'STREAK:SF': 0.5,
  'ZIGZAG:ALT': 1.5, 'ZIGZAG:ZPG': 1.6, 'ZIGZAG:SF': 0.3,
  'CHOPPY:SF': 0.4,  'BIASED:B': 1.5,  'BIASED:LC': 1.5,
};

function _getRegimeMult(regime, tag) {
  for (const [k, v] of Object.entries(_REGIME_MULT)) {
    const [r, t] = k.split(':');
...
// Dự đoán chính — logic giống 100% bản Python gốc
function predictNext(history) {
  const seq = [...history];
  if (seq.length < 8) {
    let pred;
    if (seq.length === 0) pred = Math.random() < 0.5 ? 'TAI' : 'XIU';
    else pred = seq.filter(v=>v==='TAI').length >= seq.length/2 ? 'TAI' : 'XIU';
    return { pred, conf: 51, signals: [], n_active: 0, regime: 'INIT', note: `Dữ liệu mỏng (${seq.length} phiên)` };
  }
  const regime = _detectRegime(seq);
  const Z_MIN = 2.0;
  const votes = [];
  const activeSignals = [];

  for (const [tag, fn, capW] of _SIGNALS) {
    const [acc, total, consistency] = _backtestSplit(seq, fn);
    ...
    const regimeMult = _getRegimeMult(regime, tag);
    const weight = (acc - 0.50) * 2 * capW * sampleConf * Math.min(1.0, z / 2.5) * regimeMult * consistency;
    if (weight <= 0.02) continue;
    const pTai = cur === 'TAI' ? 1.0 : 0.0;
    votes.push([pTai, weight]);
    activeSignals.push(`${tag}:${(acc*100).toFixed(0)}%/${total}`);
  }

  if (!votes.length) {
    const pred = Math.random() < 0.5 ? 'TAI' : 'XIU';
    return { pred, conf: 51, signals: [], n_active: 0, regime, consensus: 0.5 };
  }
  ...
  const wAgree = votes.filter(([p]) => (p >= 0.5) === (pTai >= 0.5)).reduce((s,[,w]) => s+w, 0);
  const consensus = wAgree / totalW;
  const conf = Math.round(51 + Math.min(edge / 0.35, 1.0) * 37);

  return {
    pred, conf,
    p_tai: +pTai.toFixed(3),
    n_active: votes.length,
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div style="font-size:13px;color:var(--t2);line-height:1.6">
        <div style="margin-bottom:8px">🧠 <span style="color:var(--t1);font-weight:600">AutoLC Prediction Engine (bản gốc)</span></div>
        <div>• 23 thuật toán: Markov (WM1-3), Streak Break/Double Break, Pattern (P3-5), Bias, Alternation, Recency, Zigzag, Momentum, Switch-rate...</div>
        <div>• Phát hiện chế độ thị trường: STREAK / ZIGZAG / BIASED / CHOPPY, điều chỉnh trọng số theo từng chế độ</div>
        <div>• Backtest split-half: kiểm tra tính nhất quán qua 2 nửa dữ liệu gần nhất trước khi tin tưởng 1 tín hiệu</div>
        <div>• Voting có trọng số theo z-score thống kê (ngưỡng Z≥2.0)</div>
...
function render(){
  ...
  if(pred&&pred.pred){
    ...
    if(rEl)rEl.textContent=pred.note||(pred.n_active?pred.n_active+' tín hiệu đồng thuận':'');
